...
Evidencia de la complejidad narrativa: El mundo de San Andreas se siente vivo porque sus personajes no existen en el vacío; operan dentro de una red densa donde las acciones (y palabras) de uno afectan a la comunidad.

Cuantificación de roles: Más allá de saber quién es el protagonista, el análisis de centralidad nos permite identificar matemáticamente quiénes son los conectores sociales, los instigadores de conflicto y los núcleos emocionales de la trama.
...
hist(bootstrap_shared_correlation$t,
  main = "Bootstrap Shared Correlation Distribution",
  xlab = "r",
  col = "#43A047",
  border = "white")
...
```{r centrality-words, fig.height=8, fig.width=12, out.width="100%"}
...
gridExtra::grid.arrange(p_closeness, p_betweenness, p_degree, p_eigen, ncol = 1)
```
...
tryCatch({
  ...
  message("Plotting failed in character-interactions: ", conditionMessage(e))
})
par(mfrow = c(1,1))
...
## 4. Conclusión
A través de este análisis computacional, hemos podido comprobar que la sensación de que el mundo de GTA San Andreas está "vivo" no es una ilusión, sino el resultado de una arquitectura narrativa robusta.

Los datos revelan que el guion no es una secuencia lineal de frases, sino un sistema dinámico. Las métricas de centralidad confirmaron que CJ actúa como un verdadero "hub" social, conectando comunidades dispares que, de otro modo, no interactuarían. Los análisis de bigramas y skip-grams mostraron cómo el lenguaje evoluciona y se adapta según el interlocutor, dotando a cada relación de una huella digital lingüística única.

En definitiva, hemos demostrado que la vitalidad de San Andreas reside en sus conexiones. Al visualizar el guion como una red de interacciones, hacemos tangible lo intangible: el juego perdura porque sus personajes tejen una red social tan compleja y reactiva como la de cualquier comunidad real. La ciencia de datos nos ha permitido ver, detrás del código del juego, el código humano que lo sostiene.
